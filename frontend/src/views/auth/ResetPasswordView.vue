...
import {useUsersStore, useNotificationStore, EMessageType, useAuthStore} from '~/stores';
...
export default defineComponent({
  setup: async function () {
    ...
    const data = {
      email: ref(user.email as string),
      password: ref(''),
      password2: ref(''),
      terms: ref(false),
      passwordRules: [
  ...
  methods: {
    validate: async function () {
      ...
      if (this.password !== this.password2) {
        ...
        notificationStore.addMessage({type: EMessageType.Danger, text: 'The given password are differents.'})
        return;
      }
